...
export default function EditPermissionPage() {
    ...
    fetch(`/api/admin/permissions?id=${id}`)
      ...
      .then(data => {
        if (data.permission) {
          ...
          setDescription(data.permission.description);
        } else {
          setMessage(data.error || "Permission not found");
        }
      })
      .catch(() => setMessage("Failed to fetch permission"))
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setMessage("");
    try {
      const res = await fetch(`/api/admin/permissions?id=${id}`, {
        method: "PUT",
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-rose-100 to-amber-100 dark:from-slate-900 dark:to-slate-800">
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl p-10 w-full max-w-md text-center">
        <h1 className="text-2xl font-bold mb-6 text-green-600 dark:text-green-400">Edit Permission</h1>
        <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
          <input className="p-2 rounded border" placeholder="Permission Name" value={name} onChange={e => setName(e.target.value)} required />
          ...
          <div className="flex gap-2 mt-2">
            <button type="submit" className="flex-1 bg-green-600 text-white py-2 rounded hover:bg-green-700 transition disabled:opacity-50" disabled={loading}>{loading ? "Saving..." : "Save"}</button>
        ...
        {message && <div className="mt-4 text-center text-sm text-green-600 dark:text-green-400">{message}</div>}
      </div>
    </div>
  );
} 